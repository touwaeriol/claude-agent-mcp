...
async function main(): Promise<void> {
  ...
  const sessionInfo = createResult.structuredContent as {
    sessionId: string;
  };
  ...
  const queryContent = (queryResult.structuredContent as Record<string, unknown>)?.finalText;
  const queryText = typeof queryContent === "string"
    ? queryContent
    : Array.isArray(queryResult.content) && queryResult.content.length > 0
    ? (queryResult.content[0] as { type?: string; text?: string }).text
    : undefined;
